...
### Internal
- Tests: 279 → 302 (+23). Two caught real defects during development: the TOML parser matched a `description = "…"` line sitting inside a `developer_instructions = """…"""` prose block, and `--no-codex` was inert because commander maps a negated flag to `opts.codex === false`, not `opts.noCodex`.
