...
## v2.13.0 변경사항 (2026-08-07)

- **변경: 시작 추정치가 비활성 플러그인을 더 이상 세지 않습니다.** 비활성 플러그인의 스킬은 세션 카탈로그에 없으니 시작 비용이 아닌데, "시작 시점 토큰"이라는 이름의 숫자에 더해지고 있었습니다. 추측이 아니라 실제 세션으로 확인했습니다 — 비활성 플러그인의 스킬은 전부 프롬프트에 없었고, 활성 플러그인의 스킬은 전부 있었습니다. 실측: **12,504 → 9,836**, 21% 보정.
- **추가: `disabledPluginSkillTokens`** — 오버헤드 줄 아래와 `--json` 에 함께 표시합니다. 전부 다시 켰을 때의 비용입니다. 헤드라인 숫자가 설명 없이 5분의 1 줄어들면 버그로 보이니까요.
- **수정: 플러그인 스킬을 마켓플레이스가 아니라 플러그인 기준으로 귀속합니다.** 한 마켓플레이스가 활성 상태가 서로 다른 플러그인 여럿을 담을 수 있어서 구분이 필요했습니다.

**명시적으로 비활성이라고 보고된** 플러그인만 제외합니다. `claude plugin list` 에는 세 번째 상태(`✘ failed to load`)가 있고 그 상태의 플러그인도 스킬은 로드되므로, 판별되지 않은 것은 그대로 셉니다.
