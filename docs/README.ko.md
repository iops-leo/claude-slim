...
## v2.11.0 변경사항 (2026-08-02)

Codex도 Claude Code와 동일한 3단계 제안·선택 정리를 받습니다. Codex가 정직하게 지원할 수 없는 두 항목만 빠집니다.

- **Codex 정리, 티어 그대로.** 7개 중 5개가 그대로 넘어옵니다. 사용 추론이 아니라 파일시스템 사실이기 때문입니다 — 깨진 심링크·빈 템플릿(Tier 1), `~/.codex/.tmp`의 설치 잔여물(Tier 1, 개발 머신에서 146MB), 백업 복사본·플러그인에 가려진 중복(Tier 2), 대용량 스킬(Tier 3). Codex 항목은 같은 목록에 `[codex]` 태그로 함께 나오고 선택 방식도 같습니다. `--no-codex`로 끌 수 있습니다.
- **`~/.codex/skills.disabled/`** 신설. 같은 `restore`로 되돌아가고, 매니페스트에 어느 에이전트 소속인지 기록합니다.
- **경로 가드를 에이전트별로 분리했습니다.** "알려진 루트 아무 데나"가 아닙니다 — Codex 이슈가 `~/.claude/`로, 그 반대로도 뻗지 못하게 막아서 조작된 매니페스트가 에이전트를 넘나들 수 없습니다.
- `unused_skill`과 `oversized_memory`는 Codex에 여전히 제공하지 않습니다. 호출 기록이 없고, `~/.codex/projects/*/memory/` 자체가 없습니다.

테스트: 347 → **374 (+27)**. 가드 스위트가 이번 릴리스의 핵심입니다 — 양방향 에이전트 격리, 경로 탈출, 그리고 단순 `startsWith`라면 통과시켰을 `~/.claude-backup` 형제 디렉터리까지 막습니다.
