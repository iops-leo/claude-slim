...
## v2.12.3 변경사항 (2026-08-07)

- **수정: 시작 총합이 플러그인 스킬을 캐시 버전마다 세고 있었습니다.** `claude plugin update`가 이전 버전을 새 버전으로의 심볼릭 링크로 남기는데(`4.9.1 -> 4.15.4`), 스캔이 둘 다 별개 설치로 따라갔습니다. 이 도구가 존재하는 이유인 바로 그 숫자가 부풀려져 있었습니다. 이제 두 스캐너 모두 세션이 실제로 로드하는 버전을 고릅니다. 실측: 플러그인 스킬 항목 148 → 107, 시작 총합 13,435 → **12,504**.
- **수정: 버전이 2개 캐시되면 플러그인별 비용이 두 배였습니다.** 브레이크다운이 한 버전의 스킬 수 옆에 두 버전의 토큰을 표시했습니다. `oh-my-claudecode`가 41개 스킬에 ~6,074 토큰으로 나왔는데 실제로는 ~3,037입니다.
- **정정: v2.12.2에서 이 문제를 "1,944 토큰, 14.5%"로 보고했지만 실제로는 931 토큰, 6.9%입니다.** 당시 추정이 같은 스킬 이름을 쓰는 별개 플러그인 둘(`document-skills`와 `example-skills`는 16개 전부 동일)을 중복으로 잘못 셌습니다.
