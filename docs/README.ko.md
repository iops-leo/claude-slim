...
## v2.12.0 변경사항 (2026-08-02)

- **`claude-slim update`** — 이 사본이 어떻게 설치됐는지에 맞는 업그레이드 명령을 **보여주고 물어본 뒤** 실행합니다. `--dry-run`으로 미리보기, `--yes`로 확인 생략. 플러그인 설치는 마켓플레이스 갱신을 먼저 하고 한정 ID(`claude-slim@claude-slim`)를 쓰며, 끝나면 재시작이 필요하다고 알립니다. npx와 소스 체크아웃은 아무것도 실행하지 않고 이유를 말합니다 — npx는 매 실행마다 최신을 받고, 사용자 저장소를 `git pull` 하는 건 claude-slim이 할 일이 아닙니다.
- **v2.9.0의 논리를 바로잡습니다.** 그때는 "업데이트는 패키지 매니저의 일"이라며 탐지에서 멈췄습니다. 절반은 맞았습니다 — `claude plugin`이 소유한 디렉터리에 직접 쓰면 안 됩니다. 나머지 절반은 틀렸습니다. 패키지 매니저를 **호출**하는 건 이 도구가 정리 과정에서 이미 하고 있던 일(`claude plugin disable`)이라, 여기서만 거부한 건 원칙이 아니라 비일관이었습니다. 직접 쓰지 않는다는 점은 그대로입니다.

테스트: 374 → **392 (+18)**. 안전 속성을 고정했습니다 — 모든 argv가 고정 리터럴이고 셸 메타문자가 없으며, 실행 파일은 `claude`나 `npm`뿐이고, TTY도 `--yes`도 없으면 실행을 거부합니다.
