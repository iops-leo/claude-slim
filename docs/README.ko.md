...
## v2.13.1 변경사항 (2026-08-22)

- **수정: `scan` 은 `~/.codex/` 를 절대 건드리지 않는다고 말하는데, `clean --auto` 는 거기서 영구 삭제를 합니다.** Codex 요약이 `Reported only — claude-slim never modifies ~/.codex/.` 로 끝나고 있었습니다. v2.10 까지는 맞는 말이었지만 `clean` 에 Codex 티어가 붙은 **v2.11** 부터는 사실이 아닙니다. 실측: `clean --dry-run` 이 `[Auto] [codex] temp_cache: .tmp (146MB of install leftovers) (permanent)` 를 선택합니다 — Tier 1 이라 `--auto` 가 묻지 않고 적용하고, `restore` 로 되돌릴 수 없습니다.
- 이제 실제로 일어나는 일을 그대로 씁니다. `scan` 은 읽기만 하고, `clean` 은 `~/.claude/` 와 동일한 티어로 여기서도 동작하며, 이동은 `~/.codex/skills.disabled/` 로 가고 `restore` 로 되돌아가지만, Tier 1 설치 잔여물은 영구 삭제됩니다.

틀린 문구를 테스트가 붙잡고 있었습니다. 요약에 "never modifies" 가 **있어야** 통과하도록 단언해둬서, 문구를 바로잡으면 테스트가 깨지는 구조였습니다. 그 문구와 모순되는 릴리스가 네 번 나가는 동안 461개 테스트는 전부 초록이었습니다. 단언을 뒤집었습니다 — 이제 요약은 `~/.codex/` 를 안 건드린다고 약속해서는 안 되고, 영구 삭제와 `--auto` 를 명시해야 합니다.
