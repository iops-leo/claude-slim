...
## v2.12.1 변경사항 (2026-08-02)

- **수정: `/claude-slim` 스킬이 프로젝트 메모리를 0으로 보고했습니다.** 스킬이 `cd "${CLAUDE_PLUGIN_ROOT}"` 로 CLI를 호출해 `cwd`가 플러그인 캐시 디렉터리가 됐고, 프로젝트 슬러그가 거기로 잡혀 아무 프로젝트와도 매칭되지 않아 **모든 프로젝트 메모리 토큰이 시작 총합에서 조용히 빠졌습니다** — 발견된 머신 기준 108,570 토큰. 슬러그를 `cwd`에서 뽑는 건 v2.8.0의 의도된 설계라 스캐너가 아니라 호출 방식이 틀린 것이었고, `SKILL.md`에서 `cd`를 없앴습니다. 이 도구의 주 사용 경로였고, 0은 깨끗한 결과처럼 보여서 티가 나지 않았습니다.
- **`--project-dir <경로>`** 추가. 프로젝트 디렉터리에서 실행할 수 없는 호출자를 위한 것이고, 자기 설치 디렉터리에서 실행 중인데 이 값이 없으면 경고합니다. 어느 프로젝트를 의도했는지 추측할 수 없으니 자신 있게 0을 보고하는 대신 말을 합니다.
