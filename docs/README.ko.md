...
## v2.9.1 변경사항 (2026-07-30)

공개 피드백을 받기 전에 스캐너를 일부러 험한 `~/.claude` 환경에 돌려보다가 발견한 것들이다.

- **한 줄이 길면 `scan`이 무한정 멈출 수 있었다.** js-tiktoken의 BPE는 **공백 없이 이어지는 구간** 길이에 대해 제곱 시간이다. 정상 산문은 문제없다(8,000자가 약 1ms) — 토크나이저가 공백으로 먼저 쪼개기 때문이다. 하지만 끊기지 않는 구간은 다르다. 실측으로 한글 800자가 약 450ms, 3,200자가 약 6.8초, 6만 자는 **출력도 에러도 없이** 20초를 넘겨 멈춰 있었다. 얼어붙은 것과 구분이 안 된다. 실제 `SKILL.md`가 base64 덩어리, 압축된 코드, 내장 JSON 스키마, 공백 없는 CJK로 이 조건에 닿는다(개발 머신의 설치된 스킬 하나가 이미 530자 스키마 한 줄을 갖고 있었다). 이제 512자를 넘는 구간은 앞부분만 인코딩해 비례 추정한다. 고정 나눗셈을 먼저 시도했다가 버렸다 — 실측 밀도가 한글 0.8자/토큰부터 반복 ASCII 8.0자/토큰까지 벌어져 최대 100% 틀렸고, 구간 자체를 표본으로 쓰면 모든 유형에서 오차 2% 이내였다. **긴 구간이 없는 파일은 빠른 경로를 타고 계산값이 한 글자도 바뀌지 않는다** — 설치된 스킬 71개로 검증해 70개가 완전 일치, 긴 구간이 있는 1개가 0.01% 이동했다. 험한 픽스처 20초+ → 1.78초, 정상 입력은 측정 가능한 차이 없음(1.68초 → 1.76초, 노이즈 범위).
- **서브커맨드를 오타 내면 엉뚱한 걸 지적했다.** `claude-slim scam`이 commander 내부 메시지인 `error: too many arguments. Expected 0 arguments but got 1`을 뱉었다. 기본 동작이 프로그램 자체에 붙어 있어 오타가 그리로 넘어간 탓이다. 이제 알 수 없는 명령어를 이름으로 지적하고 실제 명령어 목록을 보여준다.

테스트: 266 → **279 (+13)**. 새 테스트 2개가 첫 작성에서 실패해 구현을 바로잡았다 — 고정 나눗셈 추정이 반복 ASCII에서 약 100% 틀렸고, 정확도 테스트 자체가 타임아웃 났다(검증하려면 이 수정이 피하려는 그 느린 원본 인코더를 호출해야 한다).
