...
import type { ScanResult, Issue, ManifestEntry, PluginBreakdown } from './types.js';
...
export interface ReportData {
  ...
  saved: number;
  percent: number;
  topOffenders: Array<{ name: string; tokens: number }>;
  monthlySavings: number;
  sessionsPerDay: number;
  breakdown: BreakdownRow[];
  unusedPlugins: { count: number; tokens: number };
}
...
export function formatReportBox(data: ReportData): string {
  const lines: string[] = [];
  const W = 42;
...
function formatBreakdownTable(rows: BreakdownRow[]): string {
  const cols = [18, 10, 10, 12];
  const hr = '\u2500';
  const lines: string[] = [];

  const cell = (s: string, w: number, align: 'left' | 'center' = 'center'): string => {
...
export function formatScanSummary(result: ScanResult): string {
  ...
  if (result.disabledPluginSkillTokens > 0) {
    ...
    lines.push(
      `  \x1b[90mexcludes ~${result.disabledPluginSkillTokens.toLocaleString()} tokens of disabled-plugin skills — ` +
  ...
  if (result.issues.length === 0) {
    ...
    const tierColors: Record<number, string> = { 1: '31', 2: '33', 3: '37' };
  ...
  if (result.pluginBreakdown && result.pluginBreakdown.length > 0) {
    ...
    const totalEnabled = result.pluginBreakdown.filter(
      (p) => p.status !== 'disabled',
    ).length;
