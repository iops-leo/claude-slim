...
export function planUpdate(method: InstallMethod): UpdatePlan {
  switch (method) {
    case 'plugin':
      return {
        runnable: true,
    ...
    case 'npx':
      return {
        ...
        steps: [],
        guidance:
          'npx resolves the latest version on every invocation, so there is nothing to update. ' +
    ...
    case 'source':
      return {
        ...
        steps: [],
        guidance:
          'This is a source checkout. Updating it means pulling your own repository, ' +
    ...
    default:
      return {
        ...
        steps: [],
        guidance:
          'Could not tell how this copy was installed, so no upgrade command can be chosen safely. ' +
...
export function renderStep(step: UpdateStep): string {
...
function runStep(step: UpdateStep): Promise<StepResult> {
 ...
 */
export async function runUpdate(plan: UpdatePlan): Promise<StepResult[]> {
 ...
 */
export function confirmDecision(opts: { yes?: boolean }, isTTY: boolean): ConfirmDecision {
