 ...
 */
function estimateRun(segment: string, encode: (s: string) => number[]): number {
  const sample = segment.slice(0, MAX_ENCODE_RUN);
  const sampleTokens = encode(sample).length;
  if (sampleTokens === 0) return 0;
 ...
 */
function encodeBounded(text: string, encode: (s: string) => number[]): number {
