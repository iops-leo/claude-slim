...
export function detectInstallMethod(modulePath: string): InstallMethod {
  ...
  if (/\/(lib\/)?node_modules\/claude-slim\//.test(p)) return 'global';
  if (p.includes('/dist/') || p.includes('/src/')) return 'source';
  return 'unknown';
}
...
async function readCache(path: string, now: number, ttlMs: number): Promise<string | null | undefined> {
  try {
    const parsed = JSON.parse(await readFile(path, 'utf-8')) as CacheFile;
    if (parsed.version !== 1) return undefined;
    if (now - parsed.checkedAt > ttlMs) return undefined;
...
async function writeCache(path: string, now: number, latest: string | null): Promise<void> {
  const tmp = `${path}.tmp`;
  try {
    await mkdir(dirname(path), { recursive: true });
    const body: CacheFile = { version: 1, checkedAt: now, latest };
...
export function formatUpdateNotice(result: UpdateCheckResult): string | null {
  if (!result.outdated || result.latest === null) return null;
